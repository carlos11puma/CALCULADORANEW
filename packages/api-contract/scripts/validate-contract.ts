...
function collectTags(doc: Record<string, unknown>): Set<string> {
  const tags = new Set<string>();
  const paths = doc.paths;
  if (!isRecord(paths)) return tags;
  for (const pathItem of Object.values(paths)) {
    ...
    for (const operation of Object.values(pathItem)) {
      ...
      if (Array.isArray(opTags)) {
        for (const tag of opTags) if (typeof tag === "string") tags.add(tag);
      }
    }
  }
  return tags;
 ...
 * el endpoint exista.
 */
function checkBusinessRuleShapes(
  doc: Record<string, unknown>,
  issues: ValidationIssue[],
): void {
...
export function validateContract(yamlText: string): ValidationResult {
